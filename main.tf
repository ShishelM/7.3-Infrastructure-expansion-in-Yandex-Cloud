...
resource "yandex_compute_instance" "database" {
  ...
  platform_id = "standard-v3"
  zone        = "ru-central1-a" #зона ВМ должна совпадать с зоной subnet!!!
  allow_stopping_for_update = true #Останавливать при изменениях
...
resource "local_file" "inventory" {
  ...
  [webservers:vars]
  ansible_ssh_common_args='-o ProxyCommand="ssh -p 22 -W %h:%p -q pavel@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}"'
